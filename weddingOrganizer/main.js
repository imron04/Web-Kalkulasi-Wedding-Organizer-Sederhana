...
async function kirimPesanKeWA() {
  ...
  if (cartItems.length === 0 || namaInput === "" || nomorInput === "") {
    Swal.fire("Gagal", "Pastikan layanan, nama, dan nomor HP sudah diisi!", "warning");
    return;
  }
  ...
  try {
    const response = await fetch("https://script.google.com/macros/s/AKfycbxMrQM5ux3uwIciDZAB4Jn6SYDsIgO2FIH19zzVlAiKMCM84l58nsnl2DyDdXcnopVN/exec", {
      method: "POST",
      mode: "no-cors", // pakai ini untuk sementara, karena Apps Script default tidak support CORS
      headers: {
